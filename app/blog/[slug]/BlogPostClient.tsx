...
import { BlogPost, formatDate, calculateReadingTime, ALL_TOOLS, getBlogSchema, getBlogBreadcrumbSchema, getBlogFAQSchema } from '@/lib/blog';
  ...
  useEffect(() => {
    ...
    fetch('/api/blogs')
      ...
      .then(d => {
        ...
        const others = all.filter((b: BlogPost) => b.id !== blog.id);
        const relatedByTool = others.filter((b: BlogPost) => b.relatedTools?.some((t: string) => blog.relatedTools?.includes(t)));
        const relatedByCat = others.filter((b: BlogPost) => b.category === blog.category && !relatedByTool.includes(b));
